...
```{r eval=TRUE, echo=TRUE, results='asis'}
list_date_mistake <- isSurveyOnMoreThanADay(ds, 
                                                              survey_consent, 
                                                              dates, 
                                                              reportingcol, 
                                                              delete)
...
```{r eval=TRUE, echo=TRUE, results='asis'}
list_date_mistake2 <- isSurveyEndBeforeItStarts(ds, 
                                                                survey_consent, 
                                                                dates, 
                                                                reportingcol, 
                                                                delete)
...
```{r eval=TRUE, echo=TRUE, results='asis'}
list_date_mistake3 <- isSurveyMadeInTheFuture(ds, 
                                                                survey_consent, 
                                                                dates, 
                                                                reportingcol, 
                                                                delete)
...
```{r eval=TRUE, echo=TRUE, results='asis'}
list_date_mistake4 <- isSurveyStartedBeforeTheAssessment(ds, 
                                    dates,
                                    survey_consent,  
                                    start_collection, 
                                    reportingcol, 
...
list_site <- isInterviewInTheCorrectSite(adm,
                                         ds, 
                                 df_site, 
                                 ds_coord, 
                                 admin_site, 
...
```{r eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
list_sitept <- isInterviewAtTheSamplePoint(ds, 
                                pts, 
                                ds_coord, 
                                buff, 
                                survey_consent, 
...
```{r eval=TRUE, echo=TRUE, results='asis'}
list_duration_Xmin <- isInterviewTooShort(ds, 
                                     survey_consent, 
                                     dates, 
                                     reportingcol, 
                                     minduration, 
...
trackingSheet <- assessmentTrackingSheet(ds,
                              SampleSize,
                              df_site,
                              sf_site,
                              survey_consent,
...
```{r eval=TRUE, echo=TRUE, results='asis'}
list_dur <- assessmentDuration(ds, 
                                dates)
# ds <- dts
...
```{r eval=TRUE, echo=TRUE, results='asis'}
reportlog_less_X_answers <- enumeratorIsLazy(ds, 
                                           enumeratorID, 
                                           questions, 
                                           minans)
...
```{r eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
reportlog_others_values <- surveyOtherValues(ds, 
                                   otherpattern, 
                                   enumeratorID, 
                                   TRUE)
...
```{r eval=TRUE, echo=TRUE, results='asis'}
reportlog_productivity <- assessmentProductivity(ds, 
                                 surveydate, 
                                 dateformat, 
                                 survey_consent)
...
assessmentProductivityGraphical(ds, 
                                 surveydate, 
                                 dateformat, 
                                 survey_consent)
...
```{r eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
reportlog_nb_status <- assessmentDailyValidSurveys(ds, 
                              surveydate, 
                              dateformat, 
                              survey_consent)
...
```{r eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
reportlog_refusal <- enumeratorSurveysConsent(ds, 
                           survey_consent, 
                           enumeratorID)
...
```{r eval=TRUE, echo=TRUE, results='asis'}
reportlog_duration <- enumeratorSurveysDuration(ds, 
                            dates, 
                            enumeratorID)
if(nrow(reportlog_others_values[[2]]) > 0){ 
...
```{r eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='asis'}
reportlog_nb_survey <- enumeratorProductivity(ds, 
                             surveydate, 
                             enumeratorID)
...
reportlog_productivity <- enumeratorProductivityOutliers(ds, 
                                enumeratorID, 
                                surveydate, 
                                sdvalue)
if(nrow(reportlog_productivity[[2]]) > 0){ 
