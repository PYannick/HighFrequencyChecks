...
```{r eval=TRUE, echo=TRUE, results='asis'}
list[dts,err] <- chk1dii_GIS_Xm(pts, sample_dataset, df_coord, buffer, consent, reportcol, TRUE)
sample_dataset <- dts
...
```{r eval=TRUE, echo=TRUE, results='asis'}
list[dts,err] <- chk3c_date_mistake(sample_dataset, consent, dates, startdate, reportcol, FALSE)
# sample_dataset <- dts
if(nrow(err)>0){
  kable(err)
} else {
