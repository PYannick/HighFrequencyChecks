...
#'
#' @param usexlsform do not know but have to be documented
#' @param form do not know but have to be documented
#' @param ds dataset as a data.frame object
#' @param surveyConsent name as a string of the field in the dataset where the survey consent is stored
#' @param reportingColumns columns as a list of string name from the dataset you want in the result (c('col1','col2',...))
...
#' @param startDataCollection date as a string of the first day of data collection ('yyyy-mm-dd')
#' @param dateFormat format as a string used for the date ('\%m/\%d/\%Y')
#' @param minimumSurveyDuration minimum acceptable survey duration as integer in minutes
#' @param minimumSurveyDurationByIndividual minimum acceptable survey duration for one indifidual as integer in minutes
#' @param sdval number of standard deviation for which the data within is considered as acceptable
#' @param HouseholdSize name as a string of the field in the dataset where the household size is stored
#' @param deleteIsInterviewCompleted delete action to be done as a boolean (TRUE/FALSE) for isInterviewCompleted function
#' @param deleteIsInterviewWithConsent delete action to be done as a boolean (TRUE/FALSE) for isInterviewWithConsent function
#' @param correctIsInterviewInTheCorrectSite correction action to be done as a boolean (TRUE/FALSE) for isInterviewInTheCorrectSite function
#' @param deleteIsInterviewAtTheSamplePoint delete action to be done as a boolean (TRUE/FALSE) for isInterviewAtTheSamplePoint function
#' @param deleteIsUniqueIDMissing delete action to be done as a boolean (TRUE/FALSE) for isUniqueIDMissing function
#' @param deleteIsUniqueIDDuplicated delete action to be done as a boolean (TRUE/FALSE) for isUniqueIDDuplicated function
#' @param deleteIsSurveyOnMoreThanADay delete action to be done as a boolean (TRUE/FALSE) for isSurveyOnMoreThanADay function
#' @param deleteIsSurveyEndBeforeItStarts delete action to be done as a boolean (TRUE/FALSE) for isSurveyEndBeforeItStarts function
#' @param deleteIsSurveyStartedBeforeTheAssessment delete action to be done as a boolean (TRUE/FALSE) for isSurveyStartedBeforeTheAssessment function
#' @param deleteIsSurveyMadeInTheFuture delete action to be done as a boolean (TRUE/FALSE) for isSurveyMadeInTheFuture function
#' @param deleteIsInterviewTooShort delete action to be done as a boolean (TRUE/FALSE) for isInterviewTooShort function
#' @param deleteIsInterviewTooShortForTheHouseholdSize delete action to be done as a boolean (TRUE/FALSE) for isInterviewTooShortForTheHouseholdSize function
...
#' @param adminBoundaries dataset containing the shapefile - Regardless the projection used for the shapefile, it is transformed to WGS84
#' @param dsSite name as a string of the field in the dataset where the site is stored
#' @param dsCoordinates columns as a list of string name from the dataset where the GPS coordinates are stored (c('Long','Lat'))
#' @param adminBoundariesSite name as a string of the field in the shapefile where the site is stored
#' @param sampledPoints dataset containing the shapefile - Regardless the projection used for the shapefile, it is transformed to WGS84
#' @param buffer value as an integer in meter to determine the buffer from a sampled point which is acceptable
#' @param questionsSurveyBigValues columns as a list of string name from the dataset you want to check against (c(col1=x,col2=x,...))
#' @param questionsSurveySmallValues columns as a list of string name from the dataset you want to check against (c(col1=x,col2=x,...))
#' @param questionsEnumeratorIsLazy columns as a list of string name from the dataset you want to check against (c(col1=x,col2=x,...))
#' @param sampleSizeTable sampling frame as a data.frame object
#' @param sampleSizeTableSite name as a string of the field in the sampling frame where the site is stored
#' @param sampleSizeTableTarget name as a string of the field where the target number of survey is stored in the sampling frame
#' @param sampleSizeTableAvailable name as a string of the field where the number of points generated is stored in the sampling frame
#' @param consentForValidSurvey consent value for the valid surveys as string
#'
#'
...
GenerateRmd <- function(usexlsform=FALSE,
                        ...
                        dateFormat=NULL,
                        minimumSurveyDuration=NULL,
                        minimumSurveyDurationByIndividual=NULL,
                        sdval=NULL,
                        HouseholdSize=NULL,
                        deleteIsInterviewCompleted=NULL,
                        deleteIsInterviewWithConsent=NULL,
                        correctIsInterviewInTheCorrectSite=NULL,
                        deleteIsInterviewAtTheSamplePoint=NULL,
                        deleteIsUniqueIDMissing=NULL,
                        deleteIsUniqueIDDuplicated=NULL,
                        deleteIsSurveyOnMoreThanADay=NULL,
                        deleteIsSurveyEndBeforeItStarts=NULL,
                        deleteIsSurveyStartedBeforeTheAssessment=NULL,
                        deleteIsSurveyMadeInTheFuture=NULL,
                        deleteIsInterviewTooShort=NULL,
                        deleteIsInterviewTooShortForTheHouseholdSize=NULL,
                        enumeratorID=NULL,
                        enumeratorcheck=FALSE,
                        otherPattern=NULL,
...
start and end date is stored.\n
 ...
 As a result, all the check related to interview duration will be skipped ")
  ...
  if(is.null(adminBoundariesSite) | !is.character(adminBoundariesSite)){
  ...
  if(is.null(questionsSurveyBigValues) | !is.character(questionsSurveyBigValues)){
    cat("Please provide the fields you want to check for (c(field1=value,field2=value,...))")
  }
  if(is.null(questionsSurveySmallValues) | !is.character(questionsSurveySmallValues)){
    cat("Please provide the fields you want to check for (c(field1=value,field2=value,...))")
  }
  if(is.null(questionsEnumeratorIsLazy) | !is.character(questionsEnumeratorIsLazy)){
    cat("Please provide the fields you want to check for (c(field1=value,field2=value,...))")
  }
  ...
  if(is.null(minimumSurveyDurationByIndividual) | !is.numeric(minimumSurveyDurationByIndividual)){
    cat("Please provide the minimum survey time for on individual to check against")
  }
  ...
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```{r, include=TRUE}"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("sampleSizeTable <- HighFrequencyChecks::SampleSize"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("# correction for uppercase/lowercase in the site name "), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("```{r, include=TRUE}"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("# Value stored in the consent fields to indicate the survey was done "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" consentForValidSurvey <- 'yes'"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("dsCoordinates <- c(\"X_gps_reading_longitude\",\"X_gps_reading_latitude\")"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("# Name of location (__matching external geodata__)"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("sampleSizeTableSite <- \"union_name\""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("#Name of unique key in polygon file"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("adminBoundariesSite <- \"Union\""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("surveyConsent <- \"survey_consent\""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("# Variable recording multiples screening questions"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("questionsEnumeratorIsLazy <- c(consent_received.shelter_nfi.non_food_items=3,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("consent_received.food_security.main_income=3,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("consent_received.child_protection.boy_risk=3,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("consent_received.child_protection.girl_risk=3)"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("# Variable to be checked"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("reportingColumns <- c(\"enumerator_id\",\"X_uuid\")"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("startDataCollection <- \"11/11/2018\""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("minimumSurveyDuration <- 30"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("minimumSurveyDurationByIndividual <- 10"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("buffer <- 10"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("deleteIsInterviewCompleted <- FALSE                     # for isInterviewCompleted function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsInterviewWithConsent <- TRUE                    # for isInterviewWithConsent function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("correctIsInterviewInTheCorrectSite <- FALSE             # for isInterviewInTheCorrectSite function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsInterviewAtTheSamplePoint <- FALSE              # for isInterviewAtTheSamplePoint function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsUniqueIDMissing <- FALSE                        # for isUniqueIDMissing function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsUniqueIDDuplicated <- FALSE                     # for isUniqueIDDuplicated function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsSurveyOnMoreThanADay <- FALSE                   # for isSurveyOnMoreThanADay function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsSurveyEndBeforeItStarts <- FALSE                # for isSurveyEndBeforeItStarts function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsSurveyStartedBeforeTheAssessment <- FALSE       # for isSurveyStartedBeforeTheAssessment function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsSurveyMadeInTheFuture <- FALSE                  # for isSurveyMadeInTheFuture function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsInterviewTooShort <- FALSE                      # for isInterviewTooShort function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsInterviewTooShortForTheHouseholdSize <- FALSE   # for isInterviewTooShortForTheHouseholdSize function"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("list_unique_id <- isUniqueIDDuplicated(ds, uniqueID, surveyConsent)"), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```{r eval=TRUE, echo = FALSE, results='asis'}"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("list_missing_id <- isUniqueIDMissing(ds, uniqueID, surveyConsent)"), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("  surveyConsent, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("  dates)"), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("dates,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("surveyConsent,  "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("startDataCollection, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsSurveyStartedBeforeTheAssessment=deleteIsSurveyStartedBeforeTheAssessment)"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("list_site <- isInterviewInTheCorrectSite(ds,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" dsSite, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" dsCoordinates, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" adminBoundaries, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" adminBoundariesSite, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" surveyConsent, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" correctIsInterviewInTheCorrectSite=correctIsInterviewInTheCorrectSite)"), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("```"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("#### Recorded locations for each interview within a **`r buffer`** meters buffer from a sample point"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```{r eval=TRUE, echo = FALSE, message=FALSE, warning=FALSE, results='asis'}"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("list_sitept <- isInterviewAtTheSamplePoint(ds, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("dsCoordinates, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("sampledPoints, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("buffer, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("surveyConsent, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("deleteIsInterviewAtTheSamplePoint=deleteIsInterviewAtTheSamplePoint)"), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("#### Enumerators who made a survey below **`r minimumSurveyDuration`** minutes"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```{r eval=TRUE, echo = FALSE, results='asis'}"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("list_duration_Xmin <- isInterviewTooShort(ds, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" surveyConsent, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" dates, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" minimumSurveyDuration, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(" deleteIsInterviewTooShort=TRUE)"), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0("  dsSite,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("  sampleSizeTable,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("  sampleSizeTableSite,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("  sampleSizeTableTarget,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("  sampleSizeTableAvailable,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("  surveyConsent,"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("  consentForValidSurvey)"), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("#### Enumerators who pick up less than a predifined number of answers per specific questions:"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```{r eval=TRUE, echo=TRUE, results='asis'}"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("DT::datatable(data.frame(minimumAnswers=questionsEnumeratorIsLazy))"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```{r eval=TRUE, echo = FALSE, results='asis'}"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("reportlog_less_X_answers <- enumeratorIsLazy(ds, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("   enumeratorID, "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("   questionsEnumeratorIsLazy)"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  ...
  cat(paste0(" caption = paste0(\"Detected records with errors - Enumerators who pick up less than a predifined number of answers per specific questions\"))"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("} "), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0("```"), file = reportRMD , sep = "\n", append = TRUE)
  cat(paste0(""), file = reportRMD , sep = "\n", append = TRUE)
