...
#' @param output type of document generated ("html", "pdf")
#'
  ...
  for(i in 1:length(repstr[,1])){
    ...
    if(report){
      if(repstr[i,1]=="title1"){
        ...
        cat(paste0("list[var1,report", repstr[i,2], ",var3,var4]<-eval(parse(text=paste0(\"", repstr[i,2], "(\", tmp, \")\")))"), file = reportRMD , sep = "\n", append = TRUE)
        ##
        ...
        if(!is.na(repstr[i,4]) & stringi::stri_detect_fixed(repstr[i,4], "output=csv")){
          cat(paste0("write.csv(report", repstr[i,2], ", paste0(\"", repstr[i,2], "\", \".csv\"))\n"), file = reportRMD , sep = "", append = TRUE)
          cat("```", file = reportRMD , sep = "\n", append = TRUE)
          cat("Please see the generated csv file: ", repstr[i,2], ".csv\n", file = reportRMD , sep = "", append = TRUE)

        } else {
          cat(paste0("if(!is.null(report", repstr[i,2], ")){"), file = reportRMD , sep = "\n", append = TRUE)
          cat(paste0("  if(nrow(report", repstr[i,2], ")>0){"), file = reportRMD , sep = "\n", append = TRUE)
          cat(paste0("    kable(report", repstr[i,2], ")"), file = reportRMD , sep = "\n", append = TRUE)
